<?php
	...
	function sendmail($d,$m)
	{
		...
		$url= 	"https://email-sender1.p.rapidapi.com/?"."txt_msg=".rawurlencode($m)."&to=".rawurlencode($d)."&from=".rawurlencode($sendername)."&subject=".rawurlencode($subject);
		$curl = curl_init();
		...
		curl_setopt($curl,CURLOPT_HTTPHEADER,["content-type: application/json","x-rapidapi-host: email-sender1.p.rapidapi.com","x-rapidapi-key:<YOUR MAIL API>"]);
...
<body>
   ...
   <script>
    ...
    {
	    closebtns[i].addEventListener("click", function(){this.parentElement.style.display = 'none';});
    }
   </script>
</body>
</html>
